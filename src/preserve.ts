 ...
 */
export const NEVER_COMPACT: ContentType[] = [
  'decision',
  'blocker',
  'verification',
];
 ...
 */
export const ALWAYS_PRESERVE: ContentType[] = [
  'task',
  'fact',
  'decision',
  'blocker',
 ...
 */
export const MAY_COMPACT: ContentType[] = [
  'code',
  'explanation',
  'pleasantry',
  'error',
 ...
 */
export function shouldPreserve(score: ValanceScore): boolean {
 ...
 */
export function canCompact(score: ValanceScore): boolean {
 ...
 */
export function extractSessionState(items: ContentItem[]): {
  decisions: string[];
  active_tasks: string[];
  blockers: string[];
  facts: string[];
 ...
 */
export function filterPreservable(items: ContentItem[]): ContentItem[] {
 ...
 */
export function getPreservationPriority(score: ValanceScore): number {
  // Higher number = higher priority to preserve
  if (NEVER_COMPACT.includes(score.type)) return 100;
  if (ALWAYS_PRESERVE.includes(score.type)) return 80;
  if (MAY_COMPACT.includes(score.type)) return score.score;
